 ...
 * (PSID vs IGSID) and page access token differ per surface.
 */
export async function sendMetaMessage(recipientId: string, text: string): Promise<void> {
  const url = `${BASE}/me/messages?access_token=${encodeURIComponent(config.meta.pageAccessToken)}`;
 ...
 * to confirm the payload actually came from Meta and wasn't forged.
 */
